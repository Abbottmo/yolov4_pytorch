...
def yolo_forward_dynamic(output, conf_thresh, num_classes, anchors, num_anchors, scale_x_y, only_objectness=1,
    ...
    # Apply sigmoid(), exp() and softmax() to slices
    #
    bxy = torch.sigmoid(bxy) * scale_x_y - 0.5 * (scale_x_y - 1)  ## 归一化到0 到 1 之间
    bwh = torch.exp(bwh)
    det_confs = torch.sigmoid(det_confs)
    cls_confs = torch.sigmoid(cls_confs)
    ...
    # grid_x = torch.linspace(0, W - 1, W).reshape(1, 1, 1, W).repeat(1, 1, H, 1)
    # grid_y = torch.linspace(0, H - 1, H).reshape(1, 1, H, 1).repeat(1, 1, 1, W)
    #print(grid_x.shape)  (1, 1, 76, 76)   (1, 1, 38, 38)   (1, 1, 19, 19)
    #print(grid_y.shape)  (1, 1, 76, 76)   (1, 1, 38, 38)   (1, 1, 19, 19)
    anchor_w = []
    anchor_h = []
    for i in range(num_anchors):
        anchor_w.append(anchors[i * 2])
    ...
    for i in range(num_anchors):
        ...
        # print(bxy[:, ii + 1 : ii + 2] )
        # print(grid_y)
        ## 打印发现，几乎所有的xy分布都很均匀，几乎都在anchor box中心
        by = bxy[:, ii + 1 : ii + 2] + torch.tensor(grid_y, device=device, dtype=torch.float32) # grid_y.to(device=device, dtype=torch.float32)
        # Shape: [batch, 1, H, W]
        # print(bwh[:, ii : ii + 1])
        # print(anchor_w[i])
        bw = bwh[:, ii : ii + 1] * anchor_w[i]  ## 跟anchor 相乘
        ##print(bw)
        # Shape: [batch, 1, H, W]
        bh = bwh[:, ii + 1 : ii + 2] * anchor_h[i]
...
#         [1.5, 2.0, 2.375, 4.5, 5.0, 3.5]
#         [2.25, 4.6875, 4.75, 3.4375, 4.5, 9.125]
#         [4.4375, 3.4375, 6.0, 7.59375, 14.34375, 12.53125]
